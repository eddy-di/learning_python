...
class Version:
    def __init__(self, version: str) -> None:
        intified = list(map(int, version.split('.')))
        if len(intified) == 1:
            intified_list = intified + [0, 0]
            self.version = intified_list
        elif len(intified) == 2:
            intified_list = intified + [0]
            self.version = intified_list
        elif len(intified) == 3:
            self.version = intified

    def __repr__(self) -> str:
        res = ".".join(map(str, self.version))
